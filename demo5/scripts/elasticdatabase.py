...
class ElasticDatabase:
    ...
    def check_similarity(self, ref_index, image_name, fileName, file_sdhash):
        ...
        # unique tmp files
        suffix = str(time.time()).replace('.', '', 1)
        base_name = fileName.split('/')[-1]
        tmp_file_hash = base_name + suffix
        tmp_ref_hash = base_name + 'ref' + suffix

        with open(tmp_file_hash, "w") as f:
            f.write(file_sdhash)
        with open(tmp_ref_hash, "w") as f:
            f.write(ref_sdhash)

        file1 = os.path.abspath(tmp_file_hash)
        file2 = os.path.abspath(tmp_ref_hash)
        #TODO: error handling
        resline = self.__exec_cmd(['sdhash', '-c', file1, file2, '-t','0'])
        resline = resline.strip()
        score = resline.split('|')[-1]
        if score == "100":
            print fileName, ' matches 100%'
        else:
        ...
        try:
            os.remove(tmp_file_hash)
            os.remove(tmp_ref_hash)
        except:
            return
